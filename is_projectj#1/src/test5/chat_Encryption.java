...
public class chat_Encryption {
	...
	public static byte[] encrypt(String plaintxt, Key sec) //encrypt plaintext with secretkey
			throws GeneralSecurityException, NoSuchPaddingException {
		cipher = Cipher.getInstance("AES"); // encrypt message using AES Algorithm
	...
	public byte[] encrypt_seckey(Key sk, PublicKey pk){ //encrypt secretkey with publickey
		byte[] wrap=null;
		try { 
			cipher = Cipher.getInstance("RSA"); // use RSA algorithm 
			cipher.init(Cipher.WRAP_MODE, pk); //encrypt key
			 wrap = cipher.wrap(sk);
